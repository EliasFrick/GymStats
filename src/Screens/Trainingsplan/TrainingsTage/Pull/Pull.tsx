...
export default function Pull() {
    ...
    const [exercises, setExercises] = useState<ExercisePullState>(initialPullExercisesState);
    ...
    const [tBar, setTBarWightData] = useState<any>([
        0,
    ]);
    const [tightRowingWeightData, setTightRowingWightData] =
        useState<any>([0]);
    const [barbellRowflysWeightData, setBarbellRowflysWightData] = useState<any>([0]);
    const [deadliftWeightData, setDeadliftWightData] = useState<any>([0]);
    const [highRowData, setHighRowWightData] = useState<any>([0]);
    const [facePullsWeightData, setFacePullsWightData] = useState<any>([
        0,
    ]);
    const [reverseFlys, setReverseFlysWightData] = useState<any>([
        0,
    ]);
    const [pullOverWeightData, setPullOverWightData] = useState<any>([0]);
    const [szCurlsWeightData, setSzCurlsWightData] = useState<any>([0]);
    const [preacherCurlsWeightData, setPreacherCurlsWightData] = useState<any>([0]);
    const [faceAwayCurlsWeightData, setFaceAwayCurlsWightData] = useState<any>([0]);
    ...
    const [underamCurlsInsideWeightData, setUnderamCurlsInsideWightData] = useState<any>([0]);
    const [underamCurlsOutsideWeightData, setUnderamCurlsOutsideWightData] = useState<any>([0]);
    ...
    const safePopUpMenu = async (currentExercise: string) => {
        if (checkConnection) {
            try {
                ...
                const trainWeightSaver = new TrainWeightSaver(data, todoRefForUser, currentExercise)
    ...
    const getDataForChart = async (currentExercise: string) => {
        ...
        const querySnapshot = await firebasePush.firestore().collection(username).doc(getExerciseNameForDatabase(currentExercise)).collection(getExerciseNameForDatabase(currentExercise)).orderBy("timestampField", "asc").get()
    ...
    return (
        <View style={styles.container}>
            <ScrollView style={styles.BackGroundCanvas}>
                ...
                <View
                    style={[styles.Cards, {height: height * 0.12}]}
                >
                    <ChooseExerciseCards
                        title={"FaceawayCurls"}
                        ...
                        <View style={[styles.poUpContainer, {height: height * 0.1}]}>
                            <Pressable onPress={() => Keyboard.dismiss()}>
                                <View style={styles.poUptitleContainer}>
                                    <Text style={[styles.modalText, {fontSize: fontScale * 45}]}>
                                        {currentExercise}
                                    </Text>
                                </View>
                            </Pressable>
